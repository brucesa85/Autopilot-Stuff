...
[String]$RefVHD = "C:\LAB\OSDBuilder\OSImport\Windows 10 Enterprise x64 2004 19041.208\VHD\OSDBuilder.vhdx",
...
$VMLocation = "C:\LAB\Hyper-V\VMs"
#$VMMemory = "2GB"
$VHDXdiff = "$VMLocation\$VMName\$VMName-OSDisk.vhdx"
...
# Create a new VM
New-VHD -Path $VHDXdiff -ParentPath $RefVHD -differencing
Mount-DiskImage -ImagePath $VHDXdiff
$VHDXDisk = Get-DiskImage -ImagePath $VHDXdiff | Get-Disk
...
if ($CopyAutoPilotFiles)
{
    ...
    #{    If (!(Test-Path "$VHDXVolume\Windows\Provisioning\Autopilot")){ New-Item -ItemType Directory -Path "$VHDXVolume\Windows\Provisioning\Autopilot" }
            ...
            #}
    If (!(Test-Path "$VHDXVolume\Windows\Provisioning\Autopilot")){ New-Item -ItemType Directory -Path "$VHDXVolume\Windows\Provisioning\Autopilot" }
    copy-item -path $AutopilotProfile "$VHDXVolume\Windows\Provisioning\Autopilot\AutopilotConfigurationFile.json"
}

# Dismount the differencing disk
Dismount-DiskImage -ImagePath $VHDXdiff

# Create Virtual machine
New-VM -Name $VMName -BootDevice VHD -Generation 2 -MemoryStartupBytes 2GB -SwitchName $VMNetwork -Path $VMLocation -VHDPath $VHDXdiff -Verbose
